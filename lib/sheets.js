...
export async function logTryOn({ garmentId, garmentName, garmentType, subType, size }) {
  ...
  try {
    ...
    if (err?.message?.includes("invalid_grant") || err?.message?.includes("Could not load")) {
      console.error("[sheets] → Check GOOGLE_SERVICE_ACCOUNT_JSON is valid JSON and the service account has Editor access to the sheet.");
    }
  }
}
